    ...
    <>
      <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
        <div style={{ padding: '0.9rem 1rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {myRole === 'ADMIN' && (
            <button onClick={() => setShowInvite(true)} style={{ fontSize: '0.78rem', padding: '0.3rem 0.75rem', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--font-body)', background: 'var(--grad2)', color: '#fff', border: 'none', transition: 'var(--transition)' }}>
              + Invite user
            </button>
          )}
        </div>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {invitations.length === 0 && (
              <tr><td colSpan={5} style={{ padding: '2rem', textAlign: 'center', fontSize: '0.82rem', color: 'var(--text3)' }}>No invitations sent yet</td></tr>
            )}
            {invitations.map((inv) => (
              ...
              >
                ...
                {myRole === 'ADMIN' && (
                  <td style={{ padding: '0.75rem 1rem' }}>
                    {inv.status === 'PENDING' ? (
                      <button onClick={() => handleCancel(inv.id)} style={{ fontSize: '0.72rem', padding: '0.25rem 0.65rem', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--font-body)', background: 'rgba(220,38,38,0.08)', color: '#DC2626', border: '1px solid rgba(220,38,38,0.15)' }}>Cancel</button>
      ...
      <Modal open={showInvite} onClose={() => { setShowInvite(false); setQuery(''); setResults([]) }} title="Invite a user" subtitle="Search by username or email to find someone to invite.">
        ...
        {results.length > 0 && (
          <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', marginBottom: '0.5rem' }}>
            {results.map((u) => (
              <div key={u.userId} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.65rem 0.9rem', borderBottom: '1px solid var(--border)', transition: 'var(--transition)' }}
                onMouseEnter={(e) => e.currentTarget.style.background = 'var(--bg2)'}
                onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
              >
        ...
        {results.length === 0 && query && !searching && (
          <p style={{ fontSize: '0.8rem', color: 'var(--text3)', textAlign: 'center', padding: '1rem 0' }}>No users found for "{query}"</p>
        )}
        <ModalFooter>
          <Button variant="ghost" size="sm" onClick={() => { setShowInvite(false); setQuery(''); setResults([]) }}>Close</Button>
        </ModalFooter>
      </Modal>
    </>
  )
