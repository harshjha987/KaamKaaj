...
export default function InboxPanel() {
  ...
  const fetch = async () => {
    ...
    try {
      const [inv, asgn] = await Promise.all([
        invitationService.myPending(),
        assignmentService.myPending(),
      ])
  ...
  return (
    <div style={{ background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '1.25rem' }}>
      <div style={{ fontSize: '0.9rem', fontWeight: 600, fontFamily: 'var(--font-display)', letterSpacing: '-0.01em', color: 'var(--text)', marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        Inbox
        {total > 0 && (
      ...
      {loading && <div style={{ fontSize: '0.8rem', color: 'var(--text3)', textAlign: 'center', padding: '1rem' }}>Loading...</div>}
  ...
  return (
    <div style={{ padding: '0.75rem 0', borderBottom: '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', gap: '0.4rem' }}>
        <button onClick={onAccept} style={{ fontSize: '0.72rem', padding: '0.25rem 0.65rem', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--font-body)', background: 'rgba(22,163,74,0.1)', color: '#16A34A', border: '1px solid rgba(22,163,74,0.2)', transition: 'var(--transition)' }}>Accept</button>
        <button onClick={onDecline} style={{ fontSize: '0.72rem', padding: '0.25rem 0.65rem', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontWeight: 500, fontFamily: 'var(--font-body)', background: 'rgba(220,38,38,0.08)', color: '#DC2626', border: '1px solid rgba(220,38,38,0.15)', transition: 'var(--transition)' }}>Decline</button>
      </div>
    </div>
  )
}
